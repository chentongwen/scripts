...
Exec=kitten-app-player-wrapper /opt/bcm/$(json_query execName)/$(json_query execName).bcm
EOF
}
...
package(){
	...
	mkdir -p ${TEMPDIR}/opt/bcm/${NAME}
	mkdir -p ${TEMPDIR}/usr/share/icons/elementary/apps/64
	cp -r *.bcm ${TEMPDIR}/opt/bcm/${NAME}/${NAME}.bcm
...
Description: $(json_query chinacomment)
EOF
